---
type: PRD
title: "JD-from-URL Extraction"
description: "Paste a JD URL in chat and Tavily-extract it into the same /processed/<slug>.md artifact that uploaded JDs produce."
tags: [agent, files]
...
**Extends:** [Document Processing (v1)](02_document_processing.md)
...
- **Recovering from auth-walled URLs.** Tavily can't see past LinkedIn / internal Greenhouse logins, but `extract_jd` returns success on the login page itself — `/processed/<slug>.md` ends up with "Sign in to continue" instead of the JD. The agent now reads back the saved file after every successful `extract_jd` and treats it as failed when the body contains login-flow signals ("Sign in", "Log in", "Join LinkedIn", "Welcome back", etc.) or is implausibly short past the `_Source:` line. On detection, the agent offers two recovery options: save the JD as a `.txt` and re-upload (recommended — `parse_document` overwrites the bad page in place, and the agent then prepends `_Source: <original url>_` via a tiny `edit_file`), or paste the JD content into chat (handled with `write_file(path, "_Source: <url>_\n\n<pasted text>")` — write-or-replace since deepagents 0.7). Choosing a direct `write_file` over `read_file` + `edit_file` matters here: forcing the LLM to regenerate the existing login-page content as `old_string` would be expensive and error-prone for long machine-generated strings.
