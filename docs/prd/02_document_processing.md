...
9. **Parse-failure recovery.** Force a parse failure (e.g. set `LLAMA_CLOUD_API_KEY=invalid`, `docker compose restart langgraph`). Upload a PDF; agent reports the error and offers `.txt` re-upload or paste. Restore the key, re-upload as `.txt` with the same intent → `parse_document` produces `/processed/<slug>.md`. Alternatively, choose the paste option → a single `write_file` call persists the same path (the one case where the doc body legitimately flows through the model).
