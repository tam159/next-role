---
type: PRD
title: "Object Storage for Binary Artifacts"
description: "Move binary artifacts (uploads, PDFs) into S3-compatible object storage — SeaweedFS locally — behind an ObjectStoreBackend and a Python files API."
tags: [backend, storage, files]
timestamp: '2026-07-11T09:31:41+07:00'
status: "shipped"
scope: "storage layer (backend + compose + frontend file plumbing)"
version: v1
---

**Extends:** [21_own_agent_server](21_own_agent_server.md)
...
- **`ObjectStoreBackend` must NOT subclass `SandboxBackendProtocol`.** `CompositeBackend.execute` always dispatches to the *default* backend — a sandbox-flavored route would break the gating. Locked by tests. *(A second invariant originally lived here — `write` must refuse overwrite with the framework's exact "already exists" literal, which `tools._upsert` matched to fall back to `edit`. The deepagents 0.7 migration dissolved it: `write` now follows the framework-wide write-or-replace contract, `_upsert` is gone, and the backend implements recursive `delete`; the tests were updated accordingly.)*
