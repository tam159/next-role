...
2. **Enable auth**: set `AUTH_ENABLED=true` + `BETTER_AUTH_SECRET`, run the Better Auth migration (`pnpm --dir frontend dlx auth@<better-auth version> migrate --config src/lib/auth/server.ts`; existing ≤ 1.6 databases first run `frontend/scripts/better-auth-1.7-account-issuer-backfill.sql`, see README), set `LANGGRAPH_AUTH`, restart. `/` redirects to `/login`; sign up → app; `/api/auth/jwks` serves an Ed25519 key; decode `/api/auth/token` → `sub`/`iss`/`aud`/15-min `exp`.
