...
- **`backend`** — `uvicorn api.server:app` on container **:8000** (host
...
- **`core-server`** — `python -m core_server`, gRPC on **:50052** (internal only, no host
  ...
  until this is reachable (`runtime_postgres/lifespan.py` gathers
...
graph TB
    ...
    subgraph image["next-role-backend:local (one image, two commands)"]
        BE["backend<br/>uvicorn api.server:app :8000<br/>HTTP · SSE · WS · runs career_agent"]
...
**`core-server` does not hot-reload** — after editing `backend/core_server/` or
`backend/grpc_common/`, run `docker compose restart core-server`.

## 2. Package layout

| Package | LOC (approx) | Role |
|---|---|---|
| `api/` | 36k | The ASGI server: routes (assistants/threads/runs/store/crons/mcp/a2a), auth, streaming, graph loading, worker, gRPC client |
| `runtime/` | 85 | Edition router — `__init__.py` reads `LANGGRAPH_RUNTIME_EDITION` and aliases `runtime.*` submodules to the chosen backend in `sys.modules` (`postgres` → local `runtime_postgres`; `inmem` → the PyPI `langgraph-runtime-inmem` package) |
| `runtime_postgres/` | 3.5k | Postgres backend: pool + migrations, checkpoint ingestion, queue loop, store, lifespan |
| `grpc_common/` | 5.6k | Generated protobuf/gRPC stubs (`proto/`, **do not edit or lint** — see §10) + proto↔python conversion |
| `core_server/` | 2.5k | The gRPC data plane; imports only `grpc_common` |

Plus `storage/migrations/` (60 versioned SQL migrations + 2 `.lite` variants), `logging.json`
(uvicorn log config; references `api.logging.Formatter`), and `openapi.json` —
**read at import time** from the directory containing `api/`
(`api/validation.py`, `Path(__file__).parent.parent / "openapi.json"`). Moving either the
package or the file breaks server startup.

Naming: the server packages use short top-level names (`api`, `runtime`, …). They are
application packages resolved via `PYTHONPATH=/deps/next-role/backend`, never published to
PyPI, and verified not to collide with any installed distribution. Environment variables keep
their `LANGGRAPH_*` / `LANGSERVE_*` names for compatibility with the LangGraph SDK ecosystem.

The server carries **no license machinery** — NextRole is open source; `api/metadata.py` pins
`PLAN = "enterprise"` so every feature tier is always on.

## 3. The two-plane design

Everything pivots on `api/feature_flags.py`: `LANGGRAPH_RUNTIME_EDITION=postgres`
makes `IS_POSTGRES_OR_GRPC_BACKEND` true, and every HTTP handler then imports its ops layer
from `api.grpc.ops` (thin gRPC clients) instead of in-process ops. core-server is
...
directly from the backend process (`runtime_postgres/database.py`,
...
1. The backend's queue loop (`runtime_postgres/queue.py`) waits for a free
...
mounted in `api/api/event_streaming.py` behind `FF_V2_EVENT_STREAMING` (default
...
(`runtime_postgres/database.py` — `CREATE TABLE IF NOT EXISTS schema_migrations`,
...
All read in `api/config/__init__.py` unless noted. The compose file sets the starred ones.

| Env var | Default | Meaning |
|---|---|---|
| `LANGGRAPH_RUNTIME_EDITION` ★ | — (**required**; router raises) | `postgres` selects the gRPC-backed runtime (`runtime/__init__.py`) |
| `DATABASE_URI` / `POSTGRES_URI` ★ | — (**required**) | `DATABASE_URI` wins, falls back to `POSTGRES_URI`; plain `postgresql://` DSN |
| `REDIS_URI` ★ | — (**required at import**) | importing `api.config` fails without it (also true for pytest importing server modules) |
...
| `LANGSERVE_GRAPHS` ★ | — | JSON `{graph_id: "path.py:variable"}`; any source containing `/` is loaded as a file path (`api/graph.py`) |
...
| `LANGGRAPH_AUTH_TYPE` | `noop` | custom auth backends live in `api/auth/` |
...
- **api-web** — uvicorn, `N_JOBS_PER_WORKER=0`, `FF_CRONS_ENABLED=false`; scale on
  connections/CPU.
- **api-worker** — `python -m api.queue_entrypoint`, `N_JOBS_PER_WORKER=K`;
...
- **Generated code:** `grpc_common/proto/` is protoc/grpcio-tools output — never hand-edit,
