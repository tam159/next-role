...
from api.logging import LOG_LEVEL_DEBUG
from api.schema import MetadataInput
from api.serde import Fragment, Serializer, json_loads
from runtime_postgres.custom_encryption_serializer import (
...
class Checkpointer(BaseCheckpointSaver):
    ...
    def __init__(
        ...
        if api_config.LANGGRAPH_ENCRYPTION:
            from api.encryption import get_custom_encryption_instance
    ...
    async def _encrypt_json(
        ...
        """
        from api.encryption import get_encryption

        encryption = get_encryption()
        if encryption is None:
            return data
        from api.encryption.middleware import encrypt_json_if_needed
    ...
    async def _decrypt_json(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if encryption is None:
            return data
        from api.encryption.middleware import decrypt_json_if_needed
