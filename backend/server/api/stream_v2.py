...
Lives apart from :mod:`api.stream` so the v2 hot path can be
...
is in :mod:`api.event_streaming.capabilities` — the flag controls
...
from server.api.asyncio import wait_if_not_done
from server.api.feature_flags import USE_RUNTIME_CONTEXT_API
...
def normalize_protocol_event(
    ...
    *,
    ...
    (JS sidecar) path in :mod:`api.stream`. Both receive the same
    ...
    from server.api.stream import _preprocess_debug_checkpoint  # noqa: PLC0415
...
async def astream_state_v2(
    *,
    ...
    from server.api.graph import GRAPH_STREAM_TRANSFORMERS  # noqa: PLC0415
