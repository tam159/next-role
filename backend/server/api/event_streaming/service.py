...
from server.api.event_streaming.capabilities import (
    ProtocolV2Capabilities,
    ProtocolV2UnsupportedError,
    probe_event_streaming_v2_capabilities,
)
from server.api.event_streaming.constants import (
    DEFAULT_RUN_STREAM_MODES,
    EVENT_STREAMING_V2_CONFIG_KEY,
)
from server.api.event_streaming.session import (
    EventStreamingSession,
    _is_supported_channel,
)
from server.api.event_streaming.types import Subscription
from server.api.feature_flags import FF_V2_EVENT_STREAMING
from server.api.schema import MultitaskStrategy
...
class ThreadRunManager:
    ...
    async def _create_or_resume_run(
        ...
        from server.api.models.run import create_valid_run  # noqa: PLC0415
        from server.api.utils import uuid7 as make_uuid7  # noqa: PLC0415
