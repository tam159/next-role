...
from server.api.config import BG_JOB_SHUTDOWN_GRACE_PERIOD_SECS
...
async def lifespan(
    ...
    *,
    ...
    # NextRole is open source — no license verification happens at boot.

    if config.LANGGRAPH_LOGS_ENABLED:
        from server.api import self_hosted_logs
    ...
    if config.PYTHON_GRPC_SERVER_ENABLED:
        from server.api.grpc.server import (
    ...
    try:
        ...
        from server.api.grpc.client import close_shared_client
