...
- **Layout**: `backend/tests/` mirrors `backend/agents/` (so `agents/analytics_agent/` has `tests/analytics_agent/`). A source file `agents/<pkg>/<module>.py` has its tests at `tests/<pkg>/test_<module>.py` (e.g., `agents/career_agent/tools.py` → `tests/career_agent/test_tools.py`). **No `__init__.py` needed** — pytest runs in `--import-mode=importlib`, so test subdirectories are plain folders.
...
- **Artifact areas are a registry** (`object_storage.AREA_ROOTS`): each area name maps to the agent whose key space it lives in, so `/upload/` lands under `users/<scope>/career_agent/` and the analytics agent's `/charts/` under `users/<scope>/analytics_agent/`. The same mapping is the files-API allowlist. Adding an area means one entry there plus the route in that agent's `agents.py`.
